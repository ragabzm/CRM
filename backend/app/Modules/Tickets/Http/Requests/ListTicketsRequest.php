...
final class ListTicketsRequest extends FormRequest
{
    ...
    protected function prepareForValidation(): void
    {
        ...
        /*
         * `?escalated=true` normalised before validation.
         *
         * Laravel's `boolean` rule accepts 1, 0, "1", "0", true and false —
         * and NOT "true", which is what a person types into the address bar
         * and what most clients send. Worse, `(bool) "false"` is `true`, so a
         * value that slipped past validation would mean the opposite of what
         * was asked. Normalised here so the rule and the cast agree.
         */
        if ($this->has('escalated')) {
            $this->merge([
                'escalated' => filter_var(
                    $this->input('escalated'),
    ...
    public function rules(): array
    {
        return [
            ...
            /*
             * A filter, never a status. Escalation is a property an open,
